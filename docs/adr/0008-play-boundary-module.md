...
## Side fix: hole-aware `clipQuery` + the client-side post-filter that makes it correct
...
Emitting outer rings alone is not sufficient: Overpass still matches points *inside holes*, and a MultiPolygon's rings are joined into one continuous clause. Correct in/out behavior requires the client-side `contains` post-filter too. `findPlacesInZone` and `findTentacleLocations` now apply `contains` to each result element's center, dropping points that fall inside a hole (or, defense-in-depth, anywhere the server-side clause missed). This is verified by [`tests/boundary-post-filter.test.ts`](../../tests/boundary-post-filter.test.ts), which feeds a point inside a hole and asserts it is filtered out while a point in the solid ring is kept.

## Consequence

A reader can now trace "how does the boundary affect a question" through one module. The five representations collapse to three operations, each with direct unit tests ([`tests/play-boundary.test.ts`](../../tests/play-boundary.test.ts), 12 tests — up from zero), plus the post-filter test above. Adding a new boundary behavior (e.g. wiring the transit `boundaryClause`, or a "frame the map on the boundary" feature) lands in one file. This is also foundational for future architecture work: collapsing the `matching.ts`/`measuring.ts` fork-pair is easier once the boundary has a single home.
